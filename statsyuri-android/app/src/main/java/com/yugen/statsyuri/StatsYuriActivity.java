...
public class StatsYuriActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        getWindow().setStatusBarColor(DARK_BG); getWindow().setNavigationBarColor(DARK_BG); buildUi();
    }

    private void buildUi() {
        root=new FrameLayout(this); root.setBackgroundColor(DARK_BG); root.addView(new StatsBackgroundView(this),new FrameLayout.LayoutParams(-1,-1));
        LinearLayout toolbar=new LinearLayout(this); toolbar.setGravity(Gravity.CENTER_VERTICAL); toolbar.setPadding(dp(14),0,dp(8),0); toolbar.setBackgroundColor(Color.argb(242,14,18,27));
        TextView title=text("StatsYuri",19,DARK_TEXT,true); title.setGravity(Gravity.CENTER_VERTICAL); toolbar.addView(title,new LinearLayout.LayoutParams(0,dp(58),1));
        Button theme=button("☾",20); theme.setContentDescription("Toggle theme"); theme.setOnClickListener(v->toggleTheme()); toolbar.addView(theme,toolbarParams());
        Button menu=button("⋮",23); menu.setContentDescription("Settings"); menu.setOnClickListener(v->showSettings()); toolbar.addView(menu,toolbarParams());
        FrameLayout.LayoutParams tp=new FrameLayout.LayoutParams(-1,dp(58));tp.gravity=Gravity.TOP;root.addView(toolbar,tp);
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);scroll.setBackgroundColor(Color.TRANSPARENT);content=new LinearLayout(this);content.setOrientation(LinearLayout.VERTICAL);content.setPadding(dp(16),dp(14),dp(16),dp(28));scroll.addView(content);FrameLayout.LayoutParams sp=new FrameLayout.LayoutParams(-1,-1);sp.topMargin=dp(58);root.addView(scroll,sp);
        addHero();addDatasetCard();addAnalysisCard();addFooter();setContentView(root);
    }
    ...
    private void addDatasetCard(){LinearLayout card=card();card.addView(text("Dataset",20,currentText(),true));card.addView(text("CSV and XLSX are handled locally. Choose a file, then run a focused analysis or the complete analysis suite.",13,MUTED,false),margins(0,3,0,10));
        question=new EditText(this);question.setHint("What do you want to find out?  Example: compare the means of two groups");question.setHintTextColor(Color.rgb(120,130,148));question.setTextColor(currentText());question.setTextSize(15);question.setGravity(Gravity.TOP|Gravity.START);question.setMinHeight(dp(86));question.setPadding(dp(13),dp(11),dp(13),dp(11));question.setBackgroundColor(dark?Color.rgb(31,37,50):Color.rgb(239,242,247));card.addView(question,margins(0,0,0,10));
        Button choose=button("Choose CSV / XLSX dataset",15);choose.setOnClickListener(v->chooseFile());card.addView(choose,margins(0,0,0,2));fileName=text("No dataset selected",12,MUTED,false);card.addView(fileName,margins(0,2,0,8));status=text("Ready. Select a dataset to begin.",13,CYAN,true);card.addView(status,margins(0,3,0,0));content.addView(card,margins(0,0,0,12));}
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);if(requestCode!=PICK_FILE||resultCode!=RESULT_OK||data==null||data.getData()==null)return;selectedUri=data.getData();try{int flags=data.getFlags()&(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION);getContentResolver().takePersistableUriPermission(selectedUri,flags);}catch(Exception ignored){}fileName.setText(getFileName(selectedUri));status.setText("File selected. Reading locally…");new Thread(()->{try{String name=getFileName(selectedUri).toLowerCase(Locale.US);List<String[]>rows=name.endsWith(".xlsx")?readXlsx(selectedUri):readCsv(selectedUri);dataset=rows;runOnUiThread(()->status.setText("Dataset ready: "+Math.max(0,rows.size()-1)+" rows × "+(rows.isEmpty()?0:rows.get(0).length)+" columns."));}catch(Exception e){runOnUiThread(()->status.setText("Could not read file: "+e.getMessage()));}},"StatsYuri-file").start();}
    ...
    private List<String[]> readCsv(Uri uri)throws Exception{ArrayList<String[]>rows=new ArrayList<>();try(InputStream in=getContentResolver().openInputStream(uri);BufferedReader r=new BufferedReader(new InputStreamReader(in,StandardCharsets.UTF_8))){if(in==null)throw new IllegalStateException("file could not be opened");String line;while((line=r.readLine())!=null&&rows.size()<10001)if(!line.trim().isEmpty())rows.add(splitCsv(line));}return rows;}
    ...
    private List<String[]> readXlsx(Uri uri)throws Exception{ArrayList<String[]>rows=new ArrayList<>();ArrayList<String>shared=new ArrayList<>();String sheet=null;try(InputStream raw=getContentResolver().openInputStream(uri);ZipInputStream z=new ZipInputStream(raw)){ZipEntry e;while((e=z.getNextEntry())!=null){if(e.getName().equals("xl/sharedStrings.xml"))shared=parseSharedStrings(z);else if(e.getName().equals("xl/worksheets/sheet1.xml"))sheet=readText(z);}}if(sheet==null)throw new IllegalStateException("first worksheet was not found");return parseSheet(sheet,shared);}
    ...
    private void runTool(String tool){if(dataset==null||dataset.size()<2){status.setText("Choose a dataset first.");return;}status.setText("Running locally…");new Thread(()->{StatisticsEngine.Result r;try{if("auto".equals(tool))r=StatisticsEngine.focused(dataset,question.getText().toString());else if("full".equals(tool))r=StatisticsEngine.full(dataset);else if("desc".equals(tool))r=StatisticsEngine.descriptive(dataset);else if("param".equals(tool))r=new StatisticsEngine.Result("Parametric analyses",StatisticsEngine.oneSampleT(dataset,0).body+"\n\n"+StatisticsEngine.twoSampleT(dataset).body+"\n\n"+StatisticsEngine.confidenceIntervals(dataset).body);else if("anova".equals(tool))r=new StatisticsEngine.Result("ANOVA family",StatisticsEngine.oneWayAnova(dataset).body+"\n\n"+StatisticsEngine.twoWayAnova(dataset).body+"\n\n"+StatisticsEngine.tukey(dataset).body);else if("nonparam".equals(tool))r=new StatisticsEngine.Result("Non-parametric analyses",StatisticsEngine.mannWhitney(dataset).body+"\n\n"+StatisticsEngine.wilcoxon(dataset).body+"\n\n"+StatisticsEngine.kruskal(dataset).body);else if("assoc".equals(tool))r=new StatisticsEngine.Result("Association and effect sizes",StatisticsEngine.correlation(dataset).body+"\n\n"+StatisticsEngine.chiSquareGoodness(dataset).body+"\n\n"+StatisticsEngine.chiSquareIndependence(dataset).body+"\n\n"+StatisticsEngine.effectSizes(dataset).body);else if("reg".equals(tool))r=new StatisticsEngine.Result("Regression",StatisticsEngine.regression(dataset,false).body+"\n\n"+StatisticsEngine.regression(dataset,true).body);else r=new StatisticsEngine.Result("Diagnostics",StatisticsEngine.diagnostics(dataset).body+"\n\n"+StatisticsEngine.shapiro(dataset).body+"\n\n"+StatisticsEngine.levene(dataset).body+"\n\n"+StatisticsEngine.outliers(dataset).body+"\n\n"+StatisticsEngine.criticalValues().body);runOnUiThread(()->showResult(r));}catch(Exception e){runOnUiThread(()->status.setText("Analysis error: "+e.getMessage()));}},"StatsYuri-analysis").start();}
