...
if 'mavenCentral()' not in st:
    st = st.replace('repositories {', 'repositories {\n        mavenCentral()', 1)
settings.write_text(st)

m = manifest.read_text()
if 'com.chaquo.python.android.PyApplication' not in m:
    m = m.replace('<application ', '<application android:name="com.chaquo.python.android.PyApplication" ', 1)
